const rawInput = `p=24,25 v=-48,-26
p=7,8 v=-96,-2
p=7,80 v=-66,22
p=5,18 v=25,-98
p=45,67 v=26,90
...
const drawMatrix = (matrix: ReturnType<typeof createMatrix>, tick: number) => {
    ...
    const addColumn = (rowI: number, colI: number, isRobot: boolean) => {
        const row = document.getElementById(`r-${rowI}`);
...
const tick = (times: number, robots: typeof input, mH: number, mW: number) => {
    ...
    if(seen.has(JSON.stringify(robots))) clearInterval(intervalId)
...
console.log(part1(input, 103, 101, 10_000));
